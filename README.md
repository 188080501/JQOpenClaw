...
| system | system.run | 基于 QProcess 远程执行进程命令，返回 `exitCode/stdout/stderr` 等结果；支持 `command`（argv 数组）/`program+arguments`、`cwd`/`workingDirectory`、`env`/`environment`、`commandTimeoutMs`/`timeoutMs`。 |
