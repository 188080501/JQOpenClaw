...
- 配置模板：`docs/data.conf`
- Node 配置项（`config.json`）：`fileServerUrl`、`fileServerToken`

## 4. 与 Node 参数映射

Node 侧截图上传依赖两个参数：

- `fileServerUrl`：文件服务基础地址，例如 `https://files.example.com:<custom_port>`
- `fileServerToken`：上传时写入请求头 `X-Token`

说明：

- `fileServerUrl` 必须指向 Nginx 对外服务地址，不能直接填写本地磁盘路径或对象存储内部地址。
- Node 固定通过 `PUT <fileServerUrl>/upload/<filename>` 上传，并返回 `GET <fileServerUrl>/files/<filename>` 访问地址。
